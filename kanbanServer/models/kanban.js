...
const kanban = {
  ...
  async editWork(data) {
    ...
    if (!info) {
      throw new Error("수정할 작업 내역이 없습니다.");
    }

    if (info.memNo != data.memNo) {
      throw new Error("본인이 작성한 작업내역만 수정 가능합니다.");
    }

    try {
      const sql = `Update worklist Set status = :status, subject = :subject, content = :content Where idx = :idx`;
      const replacements = {
        status: data.status,
        subject: data.subject,
        content: data.content,
